...
export class App {
    ...
    setupEventListeners() {
        // UI Manager events - UPDATED with new disconnect functionality
        ...
        this.uiManager.on('device-disconnect-requested', (deviceType) => this.handleDeviceDisconnect(deviceType));
    ...
    // NEW: Device disconnect handler
    async handleDeviceDisconnect(deviceType) {
        try {
            logger.info(`🔌 Disconnecting ${deviceType}...`);
            await this.connectionManager.disconnectBluetoothDevice(deviceType);
            this.uiManager.updateDeviceStatus(deviceType, 'inactive');
            logger.info(`✅ ${deviceType} disconnected successfully`);
        } catch (error) {
            logger.error(`❌ Failed to disconnect ${deviceType}: ${error.message}`);
        }
    }
