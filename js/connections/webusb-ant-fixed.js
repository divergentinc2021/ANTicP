/**
 * WebUSB ANT+ Connection Module - QUICK FIX
...
export class WebUSBANTConnection extends EventEmitter {
    ...
    async connect() {
        // Check WebUSB API availability directly
        if (!('usb' in navigator)) {
            const error = 'WebUSB API not supported. Please use Chrome 61+ or Edge 79+.';
            logger.error(`❌ ${error}`);
        ...
        if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
            const error = 'WebUSB requires HTTPS. Please use https:// or localhost.';
            logger.error(`❌ ${error}`);
            throw new Error(error);
        }
        ...
        try {
            ...
            try {
                ...
                device = await navigator.usb.requestDevice({
                    filters: [
                        // Garmin/Dynastream ANT+ devices (your specific device)
                        { vendorId: 0x0FCF, productId: 0x1008 }, // ANT USB Stick 2 (your device)
                        ...
                        { vendorId: 0x0FCF }, // Any Dynastream/Garmin device
                        // Some clone/compatible devices
                        { vendorId: 0x1FC9 }, // NXP-based ANT+ devices
                    ]
                });
                logger.info(`📱 ANT+ device selected: ${device.productName || 'ANT+ Device'}`);
                logger.info(`🔧 Compatible with libusb-win32, libusbK, and libusb0 drivers`);
                ...
                if (selectionError.name === 'NotFoundError') {
                    ...
                    logger.info('ℹ️ WebUSB not supported - use Chrome 61+ or Edge 79+');
                }
            ...
            try {
                ...
                } else if (openError.name === 'SecurityError') {
                    logger.error('❌ WebUSB access denied - missing permissions');
                    throw new Error('WebUSB access denied. Make sure to allow USB device access.');
                }
                throw openError;
            }
            ...
            if (error.name === 'NotFoundError') {
                ...
                logger.info('  3. This works with libusbK or libusb-win32 drivers');
                logger.info('  4. Your device should show VID: 0FCF');
            } else if (error.name === 'SecurityError') {
                logger.info('💡 WebUSB Security:');
                logger.info('  • Make sure you\'re using HTTPS or localhost');
                logger.info('  • Allow USB device access when prompted');
                logger.info('  • Try refreshing the page if permissions are stuck');
                ...
                logger.info('💡 Browser Support:');
                logger.info('  • Use Chrome 61+ or Edge 79+');
                logger.info('  • Enable chrome://flags/#enable-experimental-web-platform-features');
                logger.info('  • Firefox and Safari do not support WebUSB');
            }
            throw error;
        }
    }
