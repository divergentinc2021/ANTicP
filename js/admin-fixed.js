            ...
            if (user) {
                try {
                    ...
                    if (userDoc.exists && userDoc.data().role === 'admin') {
                        ...
                        console.log('User is not admin:', user.email, 'Role:', userDoc.data()?.role);
                        resolve(null);
                    }
                } catch (error) {
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        if (currentEditUserId) {
            ...
            showAlert(`User created successfully! Default password: ${defaultPassword}`, 'success');
            resetForm();
        }
...
async function loadUsers() {
    ...
    try {
        ...
        allUsers.sort((a, b) => {
            // Sort by createdAt if it exists, otherwise put at end
        ...
        if (error.code === 'failed-precondition') {
            showAlert('Firestore index required. Check console for details.', 'error');
            console.error('You need to create a composite index for this query. Click the link below:');
            console.error(error);
        } else {
...
window.editUser = async function(userId) {
    ...
    document.getElementById('birthdate').value = user.birthdate ? new Date(user.birthdate.toDate ? user.birthdate.toDate() : user.birthdate).toISOString().split('T')[0] : '';
    ...
    if (photoPreview && user.photoURL) {
        photoPreview.innerHTML = `<img src="${user.photoURL}" alt="Profile" style="max-width: 100px; max-height: 100px;">`;
    }
...
window.deleteUser = async function(userId) {
    ...
    try {
        ...
        // Note: Deleting from Auth requires admin SDK in production
...
function resetForm() {
    ...
    const photoPreview = document.getElementById('photoPreview');
    if (photoPreview) photoPreview.innerHTML = '<span>No Photo</span>';
